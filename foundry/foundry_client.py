"""
Palantir Foundry integration client for Campaign-Guard.

Wraps the Foundry REST API with:
  - Dataset upload / download (via Foundry Catalog API)
...
class FoundryClient:
    ...
    def __init__(
        ...
        if not self.host:
            logger.warning(
                "FOUNDRY_HOST not set — Foundry calls will fail unless mocked."
            )
        if not self._token:
            logger.warning(
                "FOUNDRY_TOKEN not set — Foundry calls will fail unless mocked."
            )
    ...
    def upload_dataset(
        ...
        dataset_rid : Foundry dataset RID (ri.foundry.main.dataset.xxxxxxxx).
        branch      : Branch to write to (default "master").

        Returns
        -------
        dict with Foundry transaction metadata.
        """
        logger.info(
        ...
        try:
            ...
            logger.success(
                "Uploaded {:,} rows to {} (txn={}).", len(df), dataset_rid, txn_rid
            )
            return result
    ...
    def read_dataset(
        ...
        """
        logger.info(
            "Reading Foundry dataset {} (branch={}).", dataset_rid, branch
        )
        ...
        for file_info in parquet_files:
            path = file_info["logicalPath"]
            content = self._get_file(dataset_rid, branch, path)
            frames.append(pd.read_parquet(io.BytesIO(content)))

        df = pd.concat(frames, ignore_index=True)
        logger.success(
            "Read {:,} rows from {} ({} file(s)).",
            len(df), dataset_rid, len(frames),
        )
        return df

    def register_model(self, model_metadata: dict[str, Any]) -> dict[str, Any]:
        """
        Register a model version in the Foundry model catalog.

        Parameters
        ----------
        model_metadata : dict containing at minimum:
            name (str), version (str), framework (str), metrics (dict)
        """
        ...
        payload = {
            **model_metadata,
            ...
            "service": "campaign-guard",
        }
        logger.info(
            "Registering model '{}' v{} in Foundry catalog.",
            payload["name"], payload["version"],
        )
        url = f"{self.host}/api/v1/models"
        resp = self._post(url, json=payload)
        logger.success("Model registered. Response: {}", resp)
        return resp

    def log_predictions(self, predictions_df: pd.DataFrame) -> dict[str, Any]:
        """
        Log a batch of predictions to the Foundry predictions dataset.
        Reads PREDICTIONS_DATASET_RID from environment.
        """
        dataset_rid = os.getenv("PREDICTIONS_DATASET_RID", "")
        if not dataset_rid:
            logger.warning(
                "PREDICTIONS_DATASET_RID not set. Skipping prediction logging."
            )
            return {"status": "skipped", "reason": "PREDICTIONS_DATASET_RID not set"}
    ...
    def _open_transaction(self, dataset_rid: str, branch: str) -> dict:
        ...
        return self._post(url, json={"branchId": branch})
    ...
    def _put_file(
        self, dataset_rid: str, txn_rid: str, logical_path: str, data: bytes
    ) -> None:
        url = (
            f"{self.host}/api/v1/datasets/{dataset_rid}"
    ...
    def _post(self, url: str, **kwargs) -> dict:
        ...
        if "json" not in kwargs and "data" in kwargs:
